...
**Скриншот 1:** ЛК Yandex Cloud с созданной ВМ (виден внешний IP)

![ЛК Yandex Cloud](screenshots/yc_vm.png)

**Скриншот 2:** Подключение по SSH и `curl ifconfig.me` (IP совпадает)
 ...
 **`preemptible = true`** — делает ВМ прерываемой (в 3-4 раза дешевле). Полезно для тестовых и обучающих сред, batch-задач, которые можно перезапустить.
 **`core_fraction = 5`** — гарантированная доля vCPU (5% производительности ядра). Полезно для микросервисов с низкой нагрузкой, экономии ресурсов при обучении.
